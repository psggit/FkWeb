      ...
      {props.savedUserAddresses.map((address) => {
        return (
          <div key={address.address_id} className="address-wrap">
            <div onClick={()=>props.selectAddressFunc(address.address_id)} className="address-detail-container">
              <div className="clickableComponent">
                <div
                  className={(props.selectedAddress.address_id == address.address_id ? "selected " :"") + "radiobtn"}
                ></div>
                <div
                  className="title no-fold-text"
                >
                  {address.type}
                </div>
                <div
                  className="address line-clamp"
                >
                  {address.flat_number + ", " + address.address}
                </div>
              </div>
              <div className="modify-container">
                <div onClick={(e) => e.stopPropagation() } className="edit">EDIT</div>
                <div onClick={(e) => e.stopPropagation() }  className="delete">DELETE</div>
              </div>
            </div>
          </div>
        );
